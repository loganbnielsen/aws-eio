let test_static_resolve_no_network () =
  ...
  let t : Aws.Credentials.t =
    { source =
        Aws.Credentials.Static
          { access_key_id = "AKID"; secret_access_key = "SECRET"; session_token = None };
      region = "us-east-1" }
  in
  match Aws.Credentials.resolve ~net:env#net ~clock:env#clock ~fs:env#fs t with
  | Error e -> Alcotest.failf "expected Ok, got Error: %s" (Aws.Error.to_string e)
  | Ok r ->
...
let test_web_identity_missing_token_file () =
  ...
  | Error (Aws.Error.Credential_error msg) ->
    Alcotest.(check bool) "error mentions the token file"
      true (String.length msg > 0)
  | Error e -> Alcotest.failf "expected Credential_error, got: %s" (Aws.Error.to_string e)

let test_of_env_picks_env_chain () =
  let t = Aws.Credentials.of_env ~region:"us-west-2" () in
  Alcotest.(check bool) "source is Env_chain" true (t.source = Aws.Credentials.Env_chain);
...
let test_container_relative_uri_must_be_path_relative () =
  ...
  List.iter
    (fun relative_uri ->
      let t : Aws.Credentials.t =
        { source = Aws.Credentials.Container { relative_uri }; region = "us-east-1" }
      in
      match Aws.Credentials.resolve ~net:env#net ~clock:env#clock ~fs:env#fs t with
      | Error (Aws.Error.Credential_error _) -> ()
      | Error e -> Alcotest.failf "expected Credential_error, got: %s" (Aws.Error.to_string e)
